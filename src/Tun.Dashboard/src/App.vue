...
import type { ApiResponse, TunnelConfig, UpsertTunnelRequest } from './services/types'
...
const totalRequests = computed(() =>
  online.value.reduce((sum, item) => sum + (item.requestCount || 0), 0)
)

const onlineMap = computed(() => {
  const map = new Map()
  online.value.forEach(item => {
